...
def rnn_cell_forward_tests(target):
    ...
    a_next_tmp, yt_pred_tmp, catch_tmp = target(xt_tmp, a_prev_tmp, parameters_tmp)
    ...
    assert catch_tmp[0].shape == (5, 10), "Wrong shape in cache->a_next"
    assert catch_tmp[1].shape == (5, 10), "Wrong shape in cache->a_prev"
    assert catch_tmp[2].shape == (3, 10), "Wrong shape in cache->x_t"
    assert len(catch_tmp[3].keys()) == 5, "Wrong number of parameters in cache. Expected 5"
    ...
    a_next_tmp, yt_pred_tmp, catch = target(xt_tmp, a_prev_tmp, parameters_tmp)
